...
skill-commons 是一套可同步到 Claude Code、Codex 與 Cursor 的工程工作流程技能集。
它提供可重複執行的流程，覆蓋需求記錄、技術規格、計畫、實作、驗證與發布前檢查。v0.5.0 是公開 beta；目前的自動化驗證包含 Python CLI 與 TypeScript/Web fixtures。

此 repo 適合需要把 agent 工作落到可追蹤 artifact 的個人開發者與團隊。技能本體以 zh-TW 維護；非中文讀者可先看 [English summary](README.en.md)。

## 設計意圖

以下設計原則限制這套 workflow 的行為：

1. **LLM context 是揮發性的。** 持久狀態落在 `docs/work/<slug>/`、`meta.yml` 與 plan-sync 外部記憶，換 session 後仍能恢復。
2. **LLM 會在沒有證據時宣稱成功。** verification-before-completion 與各 Gate 要求直接證據，不接受「應該沒問題」。
3. **不可逆操作無法靠重試挽回。** push、merge、刪除前先套用 guardrails，並由 sync-work 與 branch 收尾的 Recovery Mode 控制邊界。
4. **人工審查應集中在判斷問題。** 機械檢查交給 script；人只判斷需求是否成立、設計取捨是否可接受、是否可進入發布或合併流程。
5. **交接才需要契約。** team profile 產正式 PRD、Spec、QA；personal profile 使用快照與輕量 plan。
6. **Gate 密度由 min(重做成本, 操作者的錯誤定位能力) 決定。** 非工程角色操作 prototype 時保留較多前置確認，工程流程把可機械判斷的項目自動化。
