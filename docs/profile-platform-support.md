...
## v0.9.0 membership and compatibility boundary
...
released `v0.9.0` core. `sync-work` is the sole Git-delivery owner.
Consuming repositories that still require the legacy 13-owner core stay pinned
to `v0.7.1`.

`v0.9.0` adopters add `optional` to the manifest's `capability_packs` (or use
