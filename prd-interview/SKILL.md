...
PRD 結構使用 [PRD template](resources/prd-template.md)。欄位判斷規則見
...
[PRD template](resources/prd-template.md) 與 [PRD field guide](references/prd-field-guide.md)；
...
再依 [Gate 1 reference](references/gate-1.md) 執行完整檢查。任一核心項目失敗就回到對應階段，
不得宣告 PRD 完成。
